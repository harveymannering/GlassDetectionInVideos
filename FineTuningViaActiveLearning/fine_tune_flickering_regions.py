...
gsd = PanningAugmentedGSD(gsd_path + "GSD/train/", False)
...
for e in range(epochs):
    ...
    for data in dataloader:
        ...
        # Covert output to binary segmentation map
        ...
        # Acquasition function - Choose up to pixels that are changing from GLASS to NOT GLASS (or vice versa)
        # between frames
        problem_pixels = problem_database[0, :, :, idx] + problem_pixels
        problem_pixels[problem_pixels > 0.5] = 1
        problem_database[0, :, :, idx] = problem_pixels
        gsd_ground_truth = torch.zeros((batch_size,1,384,384))
        gsd_ground_truth[0,0,:,:] = mask2 * torch.from_numpy(problem_pixels)
        gsd_ignore = torch.zeros((batch_size,1,384,384))
        gsd_ignore[0,0,:,:] = torch.abs(1 - mask2) * torch.from_numpy(problem_pixels)
        ...
        # Acquasition function - Choose all pixels that are changing from GLASS to NOT GLASS (or vice versa)
        # between frames
        problem_pixels = problem_database[1, :, :, idx] + problem_pixels
        problem_pixels[problem_pixels > 0.5] = 1
        problem_database[1, :, :, idx] = problem_pixels
        gsd_ground_truth2 = torch.zeros((batch_size,1,384,384))
        gsd_ground_truth2[0,0,:,:] = mask3 * torch.from_numpy(problem_pixels)
        gsd_ignore2 = torch.zeros((batch_size,1,384,384))
        gsd_ignore2[0,0,:,:] = torch.abs(1 - mask3) * torch.from_numpy(problem_pixels)
    ...
    for data in dataloader_original:
        ...
        debugLog("iteration" + str(total_loss_baseline))
        # Get data
        img = Variable(data[0])
        ...
        if (total_loss_baseline % 100 == 0):
            print("Epoch: " + str(e) + ",Iteration: " + str(total_loss_baseline) + ", Loss : " + str(loss.item()) + ", Transformations : " + str(transf))
        total_loss_baseline += 1
    ...
    fi = open('loss_history_flicker.csv', 'a')
    writer = csv.writer(fi)
    writer.writerow([str(e), str(total_loss_ft / (ft_count * 2)), str(ft_count), str(query_selection_count), str(total_loss_baseline / total_loss_baseline), str(np.sum(problem_database))])
    fi.close()

    # Save the model parameters every 2 epochs
    if (e + 1) % 2 == 0:
        torch.save(model.state_dict(), "Model" + str(e+1) + "-Flick")